...
def publish(PROJECT_NAME,SERVICE_NAME,SERVICE_FOLDER_NAME,MAVEN_ENV,GIT_PATH,GIT_BRANCH,
        ...
        stage('Publish') {
            ...
            for (def i = 0; i < _config.node.nodes.size(); i++) {
                ...
                node("${__node}") {
                    ...
                    __sh_target_cp_container_to_service.append(" \\mkdir -p ${_config.service.path}webapps/ROOT/;")
                    ...
                    //拼接脚本(拷贝war到目标目录，拼接重启脚本)
                    def __sh_target_restart_dubbo = new StringBuffer()
                    __sh_target_restart_dubbo.append('source /etc/profile;')
                    __sh_target_restart_dubbo.append(" rm -r ${_config.service.path}/webapps/ROOT/*;")
                    __sh_target_restart_dubbo.append(" cp ${_package_path}${_config.package.folder_name}/target/${_config.package.name} ${_config.service.path}webapps/ROOT/${_config.package.name};")
                    __sh_target_restart_dubbo.append(" unzip -o ${_config.service.path}webapps/ROOT/${_config.package.name} -d ${_config.service.path}webapps/ROOT/;")
                    ...
                    if (!__targets) {//没有目标机器，发布在facade
                        ...
                        if (!serviceContainerExist) {

                            writeFile encoding: 'utf-8', file: 'cp_container.sh', text: '''#!/bin/bash
                    ''' +
                                    __sh_target_cp_container_to_service.toString() + '''
                        '''
                            echo '拷贝容器.'
                            sh "sh cp_container.sh"
                        }

                        sh "cp ${_config.package.folder_name}/target/${_config.package.name} ${_config.service.path}webapps/"
                        //
                        writeFile encoding: 'utf-8', file: 'restart.sh', text: '''#!/bin/bash
                    ''' +
                        ...
                        for (def __targetNode in __targets) {
                            def __container_cp_sh_name = 'remote_cp_container.sh'//拷贝容器脚本名称
                            sh "ssh jhd@${__targetNode} 'mkdir -p ${_config.jenkins.tools_path}'"
//创建工具文件夹
                            sh "ssh jhd@${__targetNode} 'mkdir -p ${_config.jenkins.remote_work_path}'"
//创建工作目录
                            sh "ssh jhd@${__targetNode} 'mkdir -p ${_config.project.path}'"//创建项目目录
                            sh "ssh jhd@${__targetNode} 'mkdir -p ${_config.jenkins.remote_work_path}/${_config.package.folder_name}/target/'"
//创建包文件所在目录
                            //判断目标机器工具文件夹下启动脚本是否存在不存在就拷贝一个并解压
                            writeFile encoding: 'utf-8', file: "${__container_cp_sh_name}", text: "${__sh_target_cp_container.toString()}"
                            sh "ssh jhd@${__targetNode} 'bash -s' < ${__container_cp_sh_name}"

                            //如果没有发过服务就拷贝一份容器
                            writeFile encoding: 'utf-8', file: 'remote_cp_container.sh', text: '''#!/bin/bash
                    ''' +
                                    __sh_target_cp_container_to_service.toString() + '''
                        '''

                            echo '拷贝容器.'
                            sh "ssh jhd@${__targetNode} 'bash -s' < remote_cp_container.sh"
                            //传包到目标服务器
                            sh "scp ${_config.package.folder_name}/target/${_config.package.name} ${_config.jenkins.user}@${__targetNode}:${_package_path}${_config.package.folder_name}/target/"
